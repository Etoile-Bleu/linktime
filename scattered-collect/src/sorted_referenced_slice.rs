...
// TODO: RefSlot could become a union of ptr or (u32, u32) on 64-bit platforms.
...
/// Phase 2: sort `main` by `T`, applying the same swap pattern to `refs`.
...
pub unsafe fn initialize_scattered_sorted_referenced_slice<T: Ord>(
    ...
    refs.sort_unstable_by_key(|ref_slot| ref_slot.ptr.get());
...
//                 let refs = unsafe { REFS.as_mut_slice() };
//                 unsafe {
//                     $crate::sorted_referenced_slice::initialize_scattered_sorted_referenced_slice(main, refs);
//                 }
//             });
//         }));
...
//     __sorted_referenced_slice!(scatter TEST_SORT_REF => pub ITEM_A: u32 = 1);
//     __sorted_referenced_slice!(scatter TEST_SORT_REF => pub ITEM_B: u32 = 3);
//     __sorted_referenced_slice!(scatter TEST_SORT_REF => pub ITEM_C: u32 = 2);

//     #[test]
//     fn test_scattered_sorted_referenced_slice() {
//         assert_eq!(TEST_SORT_REF.len(), 3);
//         assert_eq!(&*TEST_SORT_REF, [1, 2, 3].as_slice());
//         for item in &TEST_SORT_REF {
//             println!("item: {}", item);
//         }
//         assert_eq!(*ITEM_A, 1);
//         assert_eq!(*ITEM_B, 3);
